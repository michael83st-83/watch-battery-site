import { createClient } from '@supabase/supabase-js';import Link from 'next/link';export const revalidate = 0;export const dynamic = 'force-dynamic';// Force Supabase to NEVER cache requests in Next.jsconst supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL!;const supabaseAnonKey = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!;const supabase = createClient(supabaseUrl, supabaseAnonKey, {global: {fetch: (url, options) => fetch(url, { ...options, cache: 'no-store' })}});// We use "any" for params to safely support the new Next.js 15 updateexport default async function WatchDetailPage({ params }: { params: any }) {// THE FIX: We must "await" the params object before trying to read the ID!const resolvedParams = await params;const watchId = Number(resolvedParams.id);const { data: watch, error } = await supabase.from('Watch Batteries').select('*').eq('id', watchId).single();if (error || !watch) {return (Watch Not Found{error ? Supabase Error: ${error.message} (Code: ${error.code}) : No watch found with ID: ${watchId} in the database.}← Back to Search);}const watchName = watch.watch_query;const batteryModel = watch['Model Number'];const requiresBattery = watch.requires_battery;const youtubeId = watch.youtube_video_id;const voltage = watch.Voltage;return (← Back to Search{watchName}  <main className="max-w-4xl mx-auto p-4 py-8 grid grid-cols-1 md:grid-cols-2 gap-6">
