...
export async function GET(request: Request) {
  ...
  // 1. If no ID is provided, output the root Sitemap Index
  if (!idParam) {
    ...
    const total = count || 0;
    // Guarantee at least 1 sitemap chunk is always generated
  ...
  // 2. If an ID is provided, output that specific chunk of watch URLs
  ...
  // Always attach the homepage to the very first chunk
  if (id === 0) {
    xml += `  <url>\n`;
    xml += `    <loc>${baseUrl}</loc>\n`;
